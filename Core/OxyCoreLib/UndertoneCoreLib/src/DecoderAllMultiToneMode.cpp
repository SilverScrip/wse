...
{
#ifdef _IOS_LOG_
  ios_log("C++ DecoderAllMultiTone Debug");
#endif //_IOS_LOG_
  ...
  //ADD member vars for all possible modes (audible, non-audible & compression)
...
{ //float *magSpectrum, float* realSpectrum, float* imagSpectrum
  ...
  for (int t = 0; t < NUM_SIMULTANEOUS_DECODING_MODES; t++)
  {
    ...
    __android_log_write(ANDROID_LOG_INFO, "OxyCoreLibInfo", text);
#endif
    ...
    __android_log_write(ANDROID_LOG_INFO, "OxyCoreLibInfo", text2);
#endif
