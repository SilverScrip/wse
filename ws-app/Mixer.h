...
public:
  Mixer(){
    ...
    mDefaultUndertoneLevel = -3.; // -3dB
    mMinUndertoneLevel = -20.f; // -20dB
  ...
  Mixer(int mode, float volumedb) {
    ...
    mDefaultUndertoneLevel = volumedb; // -3dB
    mMinUndertoneLevel = -20.f; // -20dB
    mDefaultProgramLevel = 0; // 0dB
    mSmoothTime = 0.350; // 350 ms
  ...
  ~Mixer() {};
  int mix(const float** bufferPgm, const int nsamples, int nchannels, const float samplerate, const float* bufferUndertone, float** bufferMix);
  int computeUndertoneLevel(const float* buffer, const int nsamples,  const float samplerate, std::vector<float> &timestamps, std::vector<float> &undertoneLevel, float &percentile10);
  ...
  // set functions
  void setUndertoneLevel(float gainDB){ mDefaultUndertoneLevel = gainDB;};
  void setMinUndertoneLevel(float gainDB){ mMinUndertoneLevel = gainDB;};
...
private:
  float mDefaultUndertoneLevel;
  float mDefaultProgramLevel;
  float mSmoothTime;
  float mMinUndertoneLevel;
      // flags
  int mMode;
  bool mUseNormalize;
